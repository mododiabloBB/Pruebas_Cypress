export default class AsignaturasApi {
    ...
    static consumoApiCrearAsignatura(codigo) {
        return cy.request({
            method: 'GET',
            url: `https://site2.q10.com/api/asignaturas/${codigo}?api-version=1.0`,
            headers: {
    ...
    static validarApiCrearAsignatura(codigo, nombre, abreviacion) {
        this.consumoApiCrearAsignatura(codigo).then((res) => {
            expect(res.status).to.eq(200);
            expect(res.body.Codigo).to.eq(codigo);
            expect(res.body.Nombre).to.eq(nombre);
            expect(res.body.Abreviacion).to.eq(abreviacion);
        })
    }

    static validarApiEditarAsignatura(codigo, nombre, abreviacion) {
        this.consumoApiCrearAsignatura(codigo).then((res) => {
            expect(res.status).to.eq(200);
            expect(res.body.Nombre).to.eq(nombre);
            expect(res.body.Abreviacion).to.eq(abreviacion);
